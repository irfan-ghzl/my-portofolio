...
export function PitaTipis() {
  return (
    <div aria-hidden="true" className="pita-tipis sticky top-0 z-50">
      <p className="mx-auto flex max-w-6xl items-center gap-2 px-5 py-[3px] font-mono text-[10px] leading-[1.5] tracking-[0.16em] text-perhatian-ink uppercase sm:px-8">
...
export function BlokPenyangkalan({
  ...
  return (
    <div
      ...
      className={`max-w-[70ch] border border-perhatian/60 border-l-4 border-l-perhatian bg-perhatian-soft px-4 py-3 sm:px-5 ${className}`}
    >
      <p className="flex items-start gap-3 text-[0.875rem] leading-[1.6] text-ink">
        <TriangleAlert
          className="mt-0.5 h-4 w-4 shrink-0 text-perhatian-ink"
          aria-hidden="true"
        />
        <span>
          <strong className="font-mono text-[0.75rem] font-bold tracking-[0.14em] text-perhatian-ink uppercase">
            Prototipe UI
          </strong>{" "}
